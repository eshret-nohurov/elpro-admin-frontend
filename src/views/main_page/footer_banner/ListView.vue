...
/*
 * Footer Banners List
 * Показывает нижние баннеры главной страницы с пагинацией.
 */
...
const fetchSlides = async () => {
  try {
    ...
    const response = await globalStore.makeApiRequest({
      method: 'GET',
      url: globalStore.api.endpoints.footer_banner_slides,
      params: {
...
const handleConfirm = async () => {
  try {
    ...
    await globalStore.makeApiRequest({
      method: 'DELETE',
      url: globalStore.api.endpoints.delete_footer_banner_slide(deleteData.value._id),
    })
...
<template>
  ...
  <AppList
    ...
    :listData="slides"
    :listVal="['name']"
    title="Слайды Футер Баннера"
    addButtName="Add Slide"
    addButtRoute="/main-page/footer-banner/create"
    editButtRoute="/main-page/footer-banner/edit"
    :meta="meta"
