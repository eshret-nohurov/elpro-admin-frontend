...
/*
 * Promo Banners List
 * Показывает промо-баннеры и основные действия управления.
 */
...
const fetchSlides = async () => {
  try {
    ...
    const response = await globalStore.makeApiRequest({
      method: 'GET',
      url: globalStore.api.endpoints.promo_banner_slides,
      params: {
...
const handleConfirm = async () => {
  try {
    ...
    await globalStore.makeApiRequest({
      method: 'DELETE',
      url: globalStore.api.endpoints.delete_promo_banner_slide(deleteData.value._id),
    })

    toast.success(`"${deleteData.value.name}" успешно удалена!`)
    fetchSlides()
  } catch (e) {
    console.error('Delete banner slide failed:', e)
    if (e.response.status === 401) {
      router.push('/auth/login')
    }
  } finally {
...
<template>
  ...
  <AppList
    ...
    :listData="slides"
    :listVal="['name']"
    title="Слайды Промо Баннера"
    addButtName="Add Slide"
    addButtRoute="/main-page/promo-banner/create"
    editButtRoute="/main-page/promo-banner/edit"
    :lengthLimit="slides.length >= 4 ? true : false"
