...
const fetchSubCategory = async () => {
  try {
    ...
    const response = await globalStore.makeApiRequest({
      method: 'GET',
      url: globalStore.api.endpoints.subcategory(id),
    })
    ...
    console.error('fetchSubCategory failed:', e)
...
const submitForm = async () => {
  ...
  try {
    ...
    await globalStore.makeApiRequest({
      method: 'POST',
      url: globalStore.api.endpoints.update_subcategory(id),
      data: formData,
      headers: {
    ...
    console.error('Edit subcategory failed:', e)
    if (e.response.status === 401) {
      router.push('/auth/login')
    }
  } finally {
      ...
      <form class="mt-10" @submit.prevent="submitForm">
        ...
        <div class="flex flex-wrap flex-row mt-5">
          <button
            ...
            :disabled="submitting"
          >
            {{ submitting ? 'Отправка...' : 'Изменить подкатегорию' }}
          </button>
