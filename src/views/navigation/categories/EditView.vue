...
const fetchCategory = async () => {
  try {
    ...
    const response = await globalStore.makeApiRequest({
      method: 'GET',
      url: globalStore.api.endpoints.category(id),
    })
...
const submitForm = async () => {
  ...
  try {
    ...
    await globalStore.makeApiRequest({
      method: 'POST',
      url: globalStore.api.endpoints.update_category(id),
      data: formData,
      headers: {
      ...
      <form class="mt-10" @submit.prevent="submitForm">
        ...
        <div class="flex flex-wrap flex-row mt-5">
          <button
            ...
            :disabled="submitting"
          >
            {{ submitting ? 'Отправка...' : 'Изменить категорию' }}
          </button>
          ...
          <button
            ...
            @click="$router.push('/navigation/categories')"
          >
            Отмена
          </button>
        </div>
